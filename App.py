...
def adminlogin():
    ...
    if request.method == 'POST':
        if request.form['uname'] == 'admin' and request.form['password'] == 'admin':
            conn = mysql.connector.connect(user='root', password='', host='localhost', database='5facevotedb')
            cur = conn.cursor()
...
def vreguest():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "insert into vreguesttb values('','" + vid + "','" + Reason + "','" + Address + "','waiting')")
        conn.commit()
        conn.close()
